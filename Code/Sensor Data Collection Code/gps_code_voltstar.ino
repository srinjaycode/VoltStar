...
void setup() {
    ...
    Serial1.begin(9600);  // For GPS module using hardware UART
...
// In loop(), modify IMU acceleration processing:
if (IMU.accelerationAvailable() && IMU.readAcceleration(x, y, z)) {
    float ax = x - axOffset;
    float ay = y - ayOffset;
    float az = z - azOffset;
    ...
    // If the acceleration is below a small threshold, reset the acceleration to zero
    // If the acceleration is below a small threshold, assume it's noise and reset it to zero
